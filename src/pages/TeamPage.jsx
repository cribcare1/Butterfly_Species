...
// // ─── Rank styles for top 3 (no emoji icons) ───────────────────────────────────
// const RANK_STYLES = [
//   { bg: 'linear-gradient(135deg,#f59e0b22,#f59e0b08)', border: '#f59e0b55', badge: '#f59e0b', label: '#1' },
//   { bg: 'linear-gradient(135deg,#9ca3af22,#9ca3af08)', border: '#9ca3af55', badge: '#9ca3af', label: '#2' },
//   { bg: 'linear-gradient(135deg,#b4530922,#b4530908)', border: '#b4530955', badge: '#cd7c2f', label: '#3' },
// ];
...
// function Avatar({ user, size = 42, borderColor, borderWidth = 2 }) {
...
//       background: hasImage ? 'var(--bg2)' : `${color}22`,
//       border: `${borderWidth}px solid ${borderColor || color + '55'}`,
//       display: 'flex', alignItems: 'center', justifyContent: 'center',
//       fontWeight: 700, fontSize: size * 0.22, color,
...
// // ─── Contributor card (rank 4+) ───────────────────────────────────────────────
// function ContributorCard({ user, rank }) {
//   const color = userColor(user['USER ID']);
...
//       <div style={{ flex: 1, minWidth: 0 }}>
//         <div style={{ fontSize: '.88rem', fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '.18rem' }}>
//           {user.name || user['USER ID']}
//         </div>
//         {user.name && (
//           <div style={{ fontSize: '.72rem', color: 'var(--text3)', marginBottom: '.22rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
//             {user['USER ID']}
//           </div>
//         )}
//         <div style={{ fontSize: '.72rem', color: 'var(--text3)', display: 'flex', gap: '.6rem', flexWrap: 'wrap' }}>
//           <span><span style={{ color, fontWeight: 700 }}>{user['Total Uploads By User'].toLocaleString()}</span> uploads</span>
//           <span>·</span>
//           <span><span style={{ color: 'var(--text2)', fontWeight: 600 }}>{(user.valued_uploads || 0).toLocaleString()}</span> valued</span>
//         </div>
//       </div>
//     </div>
//   );
...
//                     onMouseLeave={e => e.currentTarget.style.transform = 'none'}
//                   >
//                     {/* Rank label (text, no icon) */}
//                     <div style={{
//                       fontSize: '.72rem', fontWeight: 800, letterSpacing: '.08em',
//                       color: rs.badge, marginBottom: '.75rem', textTransform: 'uppercase',
//                     }}>
//                       {rs.label}
//                     </div>
...
//                       <Avatar user={u} size={64} borderColor={rs.badge} borderWidth={2.5} />
//                     </div>

//                     {/* Name */}
//                     <div style={{ fontFamily: 'var(--ff)', fontSize: '.95rem', fontWeight: 600, color: 'var(--text)', marginBottom: '.15rem', wordBreak: 'break-word' }}>
//                       {u.name || u['USER ID']}
//                     </div>
//                     {u.name && (
//                       <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginBottom: '.55rem' }}>
//                         {u['USER ID']}
//                       </div>
//                     )}
...
//                       {u['Total Uploads By User'].toLocaleString()}
//                     </div>
//                     <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginTop: '.2rem', marginBottom: '.6rem' }}>uploads</div>

//                     {/* Valued uploads pill */}
//                     <div style={{
//                       display: 'inline-flex', alignItems: 'center', gap: '.3rem',
//                       background: `${rs.badge}18`, border: `1px solid ${rs.badge}44`,
//                       borderRadius: 20, padding: '.2rem .65rem',
//                       fontSize: '.7rem', color: rs.badge, fontWeight: 600,
//                     }}>
//                       <span style={{ fontSize: '.6rem', opacity: .7 }}>★</span>
//                       {(u.valued_uploads || 0).toLocaleString()} valued
//                     </div>
//                   </div>
//                 );
//               })}
...
//           <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '.85rem' }}>
//             {displayed.map((u, i) => (
//               <ContributorCard key={u['USER ID']} user={u} rank={i + 3} />
//             ))}
//           </div>
...
const RANK_STYLES = [
  { bg: 'linear-gradient(135deg,#f59e0b22,#f59e0b08)', border: '#f59e0b55', badge: '#f59e0b' },
  { bg: 'linear-gradient(135deg,#9ca3af22,#9ca3af08)', border: '#9ca3af55', badge: '#9ca3af' },
  { bg: 'linear-gradient(135deg,#b4530922,#b4530908)', border: '#b4530955', badge: '#cd7c2f' },
];
...
// ─── Avatar ───────────────────────────────────────────────────────────────────
// objectFit: 'contain' so the full image is always visible, no cropping.
// Background matches the card so it looks clean if the image has letterboxing.
function Avatar({ user, size = 42, borderColor, borderWidth = 2 }) {
  const [imgError, setImgError] = useState(false);
  const color    = userColor(user['USER ID']);
  const hasImage = user.profile_image && !imgError;
  ...
  return (
    <div style={{ padding: '7rem 1.5rem 4rem', maxWidth: 1200, margin: '0 auto' }}>
      ...
      {loading ? (
        ...
        <>
          {/* ── Podium top 3 — no rank badge, full images ── */}
          {contributors.length >= 3 && (
            <div
              style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem', marginBottom: '2.5rem', alignItems: 'end' }}
              className="podium-grid"
            >
              {[1, 0, 2].map(i => {
                const u          = contributors[i];
                const rs         = RANK_STYLES[i];
                const avatarSize = i === 0 ? 120 : 96;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 20,
                      padding:      `${i === 0 ? '2.5rem' : '2rem'} 1.5rem 1.75rem`,
                      textAlign:    'center',
                      transition:   'transform .22s',
                    }}
                    ...
                    onMouseLeave={e => e.currentTarget.style.transform = 'none'}
                  >
                    {/* Avatar — full image with contain */}
                    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
                      <Avatar
                        user={u}
                        size={avatarSize}
                        borderColor={rs.badge}
                    ...
                    {/* Name */}
                    <div style={{
                      fontFamily: 'var(--ff)', fontSize: i === 0 ? '1.05rem' : '.92rem',
                      fontWeight: 600, color: 'var(--text)',
                      marginBottom: '.15rem', wordBreak: 'break-word',
                    }}>
                      {u.name || u['USER ID']}
                    </div>
                    {u.name && (
                      <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginBottom: '.6rem' }}>
                        {u['USER ID']}
                      </div>
                    )}
                    ...
                    {/* Upload count */}
                    <div style={{
                      fontSize:   i === 0 ? '2rem' : '1.6rem',
                      fontWeight: 700, color: rs.badge,
                      fontFamily: 'var(--ff)', lineHeight: 1,
                    }}>
                      {u['Total Uploads By User'].toLocaleString()}
                    </div>
                    <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginTop: '.2rem', marginBottom: '.7rem' }}>
                      uploads
                    </div>
                    ...
                    <div style={{
                      display:      'inline-flex', alignItems: 'center', gap: '.3rem',
                      background:   `${rs.badge}18`, border: `1px solid ${rs.badge}44`,
                      borderRadius: 20, padding: '.22rem .7rem',
                      fontSize:     '.7rem', color: rs.badge, fontWeight: 600,
                    }}>
                      <span style={{ fontSize: '.6rem', opacity: .7 }}>★</span>
                      {(u.valued_uploads || 0).toLocaleString()} valued
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '.85rem' }}>
            {displayed.map((u, i) => (
              <ContributorCard key={u['USER ID']} user={u} />
            ))}
          </div>
