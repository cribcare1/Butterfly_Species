...
// import Loader from '../components/shared/Loader';
// import IndiaMap from '../components/map/IndiaMap';
// import { wikiImageUrl } from '../utils/appUtils';
...
// // ─── Build display cards from taxonomy detail files ────────────────────────────
// /**
//  * Converts a flat list of file objects (from tree-search) into card objects
//  * that SpeciesCard can render. Each file becomes one card.
//  */
// function buildCardsFromFiles(files, familyColor = '#52c97b') {
//   if (!Array.isArray(files) || files.length === 0) return [];
//   return files
//     .filter(f => {
//       // Must have at least one image reference
//       return f.file_url || f.img_name || f.file_name;
//     })
//     .map((f, i) => {
//       const fileName    = f.file_name || f.img_name || '';
//       const displayName = fileName.replace(/_/g, ' ').replace(/\.(jpg|jpeg|png|webp)$/i, '');
//       const imageUrl    = resolveImageUrl(f, 400);
//       return {
//         id:          `taxon-file-${i}-${fileName}`,
//         name:        displayName || 'Unknown Species',
//         scientific:  f.species || f.scientific_name || displayName || '',
//         region:      f.state || f.district || f.country || '',
//         family:      f.family || '',
//         color:       familyColor,
//         imageUrl,
//         _images:     [f],
...
//       }}>
//         <strong style={{ color: '#fff' }}>{speciesName}</strong>
//         {resolveCaption(img) && <>{' · '}{resolveCaption(img)}</>}
//         {images.length > 1 && (
//           <span style={{ color: 'var(--text3)', marginLeft: '0.5rem' }}>
//             {idx + 1} / {images.length}
//           </span>
...
//               {species.date && (
//                 <span style={{
//                   display: 'inline-block', marginTop: '.4rem', marginLeft: '.4rem',
//                   fontSize: '.66rem', padding: '.18rem .55rem',
...
//           {(species.region || species.status || species.subcategory) && (
//             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.5rem', marginBottom: '1.25rem' }}>
//               {species.status     && <Chip label="Status"   value={species.status}     valueColor="#52c97b" />}
//               {species.region     && <Chip label="Region"   value={species.region}     />}
//               {species.subcategory && <Chip label="Subfamily" value={species.subcategory} />}
//               {species.wingspan   && <Chip label="Wingspan" value={species.wingspan}   />}
//             </div>
//           )}
...
// function SpeciesCard({ s, isSelected, onClick }) {
//   const imgEntry = Array.isArray(s._images) && s._images.length > 0 ? s._images[0] : null;
//   const imageUrl = imgEntry ? resolveImageUrl(imgEntry, 400) : s.imageUrl;
...
//           position: 'absolute', inset: 0,
//           alignItems: 'center', justifyContent: 'center',
//           fontSize: '3rem', background: `${s.color || '#333'}18`,
//         }}>🦋</div>
...
//             padding: '.15rem .5rem', borderRadius: 20, fontSize: '.63rem',
...
//         <div style={{
//           fontFamily: 'var(--ff)', fontSize: '1rem', color: 'var(--text)',
//           marginBottom: '.12rem',
//           whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
//         }}>
//           {s.name}
//         </div>
//         {s.scientific && s.scientific !== s.name && (
//           <div style={{ fontSize: '.72rem', color: 'var(--text3)', fontStyle: 'italic', marginBottom: '.5rem' }}>
//             {s.scientific}
//           </div>
//         )}
//         <div style={{ display: 'flex', gap: '.4rem', flexWrap: 'wrap', marginBottom: '.6rem' }}>
//           {s.region && (
//             <span className="tag t-blue" style={{ fontSize: '.63rem' }}>{s.region}</span>
//           )}
//           {s.family && (
//             <span className="tag t-purple" style={{ fontSize: '.63rem' }}>{s.family}</span>
//           )}
//         </div>
//         <div style={{
...
//         {Array.from({ length: 6 }).map((_, i) => (
//           <div key={i} className="card" style={{ padding: 0, overflow: 'hidden', opacity: 0.45 }}>
//             <div style={{ width: '100%', height: 180, background: 'var(--bg3)', position: 'relative', overflow: 'hidden' }}>
//               <div style={{
//                 position: 'absolute', inset: 0,
//                 background: 'linear-gradient(90deg, transparent, rgba(255,255,255,.04), transparent)',
//                 backgroundSize: '200% 100%', animation: 'shimmer 1.6s infinite',
...
// // ─── Taxonomy detail loader skeleton ──────────────────────────────────────────

// function TaxonLoader({ label }) {
//   return (
//     <div style={{ padding: '3rem 1rem', textAlign: 'center' }}>
//       <Loader />
//       <p style={{ marginTop: '1rem', color: 'var(--text2)', fontFamily: 'var(--ff)', fontStyle: 'italic' }}>
//         Loading {label || 'species'}…
//       </p>
//     </div>
//   );
// }
...
//   const isInitialLoading = state.familiesLoadingCount > 0;
//   const loadedCount      = 6 - state.familiesLoadingCount;

//   // ── Determine what to show in the grid ──────────────────────────────────────
//   //
//   // Priority order:
//   //  1. A sidebar leaf node was clicked → selectedSpeciesFilter is set
//   //     → show taxonomyDetailFiles (tree-search result for that category)
//   //  2. A family header was clicked (selectedCategory set, no filter)
//   //     → show taxonomyDetailFiles for the whole family if available,
//   //       else fall back to the subtree leaf list (names only, no images yet)
//   //  3. No selection → show images from the first family's tree-search
//   //     (auto-triggered below via useEffect)
//   //
//   const filterCategory = state.selectedSpeciesFilter?.category;
//   const isTaxonLoading = state.taxonomyDetailLoading;

//   // Resolve the colour for the currently active family
//   const activeFamilyColor = selCat?.color || '#52c97b';

//   // Cards built from tree-search files
//   const taxonCards = buildCardsFromFiles(state.taxonomyDetailFiles, activeFamilyColor);

//   // ── Auto-load the first family on mount so something is visible ─────────────
//   useEffect(() => {
//     // Only trigger when: no filter is active, no detail already loaded/loading,
//     // and categories have been populated.
//     if (
//       !state.selectedSpeciesFilter &&
//       !state.taxonomyDetail &&
//       !state.taxonomyDetailLoading &&
//       state.categories.length > 0
//     ) {
//       const firstCat = state.categories[0]; // Papilionidae
//       dispatch({ type: 'SEL_CAT', v: firstCat });
//       dispatch({
//         type: 'SEL_SPECIES_FILTER',
//         v: { category: firstCat.name, label: firstCat.name, path: [firstCat.name] },
//       });
//     }
//   // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [state.categories.length]);

//   // ── Breadcrumb label for the current filter ─────────────────────────────────
//   const filterLabel = state.selectedSpeciesFilter?.label || state.selectedSpeciesFilter?.category || '';

//   // ── Display species: taxonCards when available, else empty ──────────────────
//   const displayCards = taxonCards;
...
//         <div style={{ display: 'flex', alignItems: 'center', gap: '.4rem', fontSize: '.78rem', color: 'var(--text2)', flexWrap: 'wrap' }}>
//           <span
//             style={{ cursor: 'pointer', color: !selCat && !filterCategory ? '#52c97b' : 'var(--text3)' }}
//             onClick={() => {
//               dispatch({ type: 'SEL_CAT', v: null });
//               dispatch({ type: 'SEL_SPECIES_FILTER', v: null });
//             }}
...
//               <span
//                 style={{
//                   color: filterCategory && filterCategory !== selCat.name ? 'var(--text3)' : '#52c97b',
//                   cursor: 'pointer',
//                 }}
//                 onClick={() => {
//                   dispatch({ type: 'SEL_SUBCAT', v: 'all' });
//                   dispatch({
//                     type: 'SEL_SPECIES_FILTER',
//                     v: { category: selCat.name, label: selCat.name, path: [selCat.name] },
//                   });
//                 }}
//               >
//                 {selCat.name}
...
//           {selCat && (state.selectedSubcatPath || []).map((crumb, i) => {
//             const isLast   = i === state.selectedSubcatPath.length - 1;
//             const isActive = isLast && (!filterCategory || filterCategory === crumb.category);
//             return (
//               <span key={crumb.category} style={{ display: 'flex', alignItems: 'center', gap: '.4rem' }}>
//                 <span style={{ color: 'var(--text3)' }}>›</span>
//                 <span
...
//                       dispatch({
//                         type: 'SEL_SPECIES_FILTER',
//                         v: { category: crumb.category, label: crumb.label, path: state.selectedSubcatPath.slice(0, i + 1).map(c => c.category) },
//                       });
//                     }
//                   }}
//                 >
...
//           {/* Leaf filter crumb (when it differs from the last subcat path entry) */}
//           {filterLabel && filterCategory && filterCategory !== selCat?.name &&
//             !(state.selectedSubcatPath || []).some(c => c.category === filterCategory) && (
//             <>
//               <span style={{ color: 'var(--text3)' }}>›</span>
//               <span style={{ color: '#52c97b' }}>{filterLabel}</span>
//             </>
//           )}
//         </div>
...
//         <span style={{ fontSize: '.78rem', color: 'var(--text3)' }}>
//           {isTaxonLoading ? 'Loading…' : isInitialLoading ? 'Loading…' : `${displayCards.length} photos`}
//         </span>
//       </div>
...
//         <IndiaMap sightings={state.sightings} loading={state.sightingsLoading} />
//       ) : isTaxonLoading ? (
//         <TaxonLoader label={filterLabel || selCat?.name} />
//       ) : (
//         <>
//           {/* Taxonomy detail error */}
//           {state.taxonomyDetailError && (
//             <div style={{
//               padding: '1rem 1.5rem', marginBottom: '1rem',
//               background: 'rgba(248,113,113,.08)', border: '1px solid rgba(248,113,113,.3)',
//               borderRadius: 10, color: '#f87171', fontSize: '.82rem',
//             }}>
//               ⚠ Failed to load images: {state.taxonomyDetailError}
//             </div>
//           )}
...
//           {displayCards.length === 0 && !isTaxonLoading && (
//             <div style={{ textAlign: 'center', padding: '5rem 1rem', color: 'var(--text3)' }}>
//               <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>🦋</div>
//               <p style={{ fontFamily: 'var(--ff)', fontSize: '1.1rem' }}>
//                 {filterLabel
//                   ? `No photos uploaded yet for ${filterLabel}.`
//                   : 'Select a family or species from the sidebar.'}
//               </p>
//             </div>
//           )}
//         </>
...
//               }}
//             >✕</button>
//             <h2 style={{ fontFamily: 'var(--ff)', fontSize: '1.5rem', color: 'var(--text)', marginBottom: '1rem' }}>
//               Filter Species
//             </h2>
//             <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
//               <button
...
function resolveImageUrl(img, width = 400) {
  if (!img) return null;
  // FIX: added image_url as a fallback field — some API responses use this key
  if (img.file_url)   return wikiImageUrl(img.file_url,   width);
  if (img.image_url)  return wikiImageUrl(img.image_url,  width);
  if (img.img_name)   return wikiImageUrl(img.img_name,   width);
  if (img.file_name)  return wikiImageUrl(img.file_name,  width);
  return null;
}
...
function buildCardsFromFiles(files, familyColor = '#52c97b') {
  ...
  return files
    .filter(f => {
      // FIX: include image_url as a valid image source
      return f.file_url || f.image_url || f.img_name || f.file_name;
    })
    .map((f, i) => {
      const fileName    = f.file_name || f.img_name || '';
      const displayName = fileName.replace(/_/g, ' ').replace(/\.(jpg|jpeg|png|webp)$/i, '');
...
export default function SpeciesPage() {
  ...
  // ── Auto-load the first family on mount ─────────────────────────────────────
  // FIX: added guard so this only fires once, after categories are populated,
  // and only when no filter is already active (avoids overwriting user selection)
  useEffect(() => {
    if (
      state.categories.length === 0 ||       // categories not ready yet
      state.selectedSpeciesFilter ||          // user already has an active selection
      state.taxonomyDetail ||                 // detail already loaded
      state.taxonomyDetailLoading             // fetch already in flight
    ) return;

    const firstCat = state.categories[0];
    if (!firstCat) return; // FIX: guard against empty categories array
