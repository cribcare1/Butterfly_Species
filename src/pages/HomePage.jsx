...
import ncsLogo from '../assets/ncs-logo.jpeg';
import wingsLogo from '../assets/wings-logo.jpeg';
import bigButterflyLogo from '../assets/big-butterfly-month.jpeg';
import easternHorizons from '../assets/eastern-horizons.jpeg';
import wbWikimedians from '../assets/wb-wikimedians.jpeg';
import thamblyokLogo from '../assets/thamblyok.jpeg';
import bssSikkimLogo from '../assets/butterfly-society-sikkim.png';
import pbvmLogo from '../assets/pvbm-logo.png';
import wildFeathersLogo from '../assets/wildfeathers-black.png';
    ...
    >
      ...
      >
        {extended.map((img, i) => {
          ...
          return (
            <div key={i} style={{ minWidth: '100%', width: '100%', height: '70vh', position: 'relative', flexShrink: 0, overflow: 'hidden' }}>
              {baseUrl ? (
                <img
                  src={baseUrl}
              ...
              <div style={{ display: baseUrl ? 'none' : 'flex', height: '100%', alignItems: 'center', justifyContent: 'center', fontSize: '4rem', background: 'var(--bg3)' }}>🦋</div>
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '30%', background: 'linear-gradient(to top, rgba(0,0,0,.75) 0%, transparent 100%)', pointerEvents: 'none' }} />
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.5rem' }}>
                <div style={{ fontFamily: 'var(--ff)', fontSize: '1.2rem', fontWeight: 600, color: '#fff', marginBottom: 4 }}>{img.display_name}</div>
        ...
        <button
          ...
          onClick={() => {
            if (dir === -1 && current === 0) {
              ...
              setTimeout(() => { setIsTransitioning(true); setCurrent(images.length - 1); startAuto(); }, 50);
              return;
            }
            goTo(current + dir);
          }}
          style={{ position: 'absolute', top: '50%', [side]: 12, transform: 'translateY(-50%)', background: 'rgba(255,255,255,.15)', border: '1px solid rgba(255,255,255,.3)', borderRadius: '50%', width: 40, height: 40, cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2 }}
        >{label}</button>
      ))}

      <div style={{ position: 'absolute', bottom: '1rem', right: '1.5rem', display: 'flex', gap: 6, zIndex: 2 }}>
        {images.map((_, i) => (
          <button key={i} onClick={() => goTo(i)}
            style={{ width: activeDot === i ? 20 : 6, height: 6, borderRadius: 3, border: 'none', cursor: 'pointer', background: activeDot === i ? '#fff' : 'rgba(255,255,255,.4)', transition: 'all .3s', padding: 0 }} />
        ))}
      </div>

      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, background: 'rgba(255,255,255,.2)', zIndex: 2 }}>
        <div key={current} style={{ height: '100%', background: '#fff', animation: 'carouselProgress 3s linear forwards' }} />
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// PARTNERS DATA
// darkBg: true  → logo has a dark/black background, use dark container
// darkBg: false → logo has a light background, use white container
// ─────────────────────────────────────────────────────────────────────────────

const PARTNERS = [
  { name: 'BoRS',                fullName: 'Biodiversity of Rabindra Sarovar',            logo: borsLogo,         href: '#',                               darkBg: false },
  { name: 'Nature Mates',        fullName: 'Nature Club',                                 logo: natureMatesLogo,  href: 'https://naturematessociety.org',   darkBg: false },
  { name: 'NCS',                 fullName: 'Nature Conservation Society',                 logo: ncsLogo,          href: '#',                               darkBg: true  },
  { name: 'WINGS',               fullName: 'Wildlife Information & Nature Guide Society', logo: wingsLogo,        href: '#',                               darkBg: false },
  { name: 'Big Butterfly Month', fullName: 'Annual Citizen Science Count',                logo: bigButterflyLogo, href: '#',                               darkBg: false },
  { name: 'Eastern Horizons',    fullName: 'Discover North East With Us',                 logo: easternHorizons,  href: '#',                               darkBg: true  },
  { name: 'WB Wikimedians',      fullName: 'West Bengal Wikimedians',                     logo: wbWikimedians,    href: '#',                               darkBg: false },
  { name: 'Thamblyok',           fullName: 'Butterfly Society of North Sikkim',           logo: thamblyokLogo,    href: '#',                               darkBg: false },
  { name: 'BSS',                 fullName: 'Butterfly Society of Sikkim',                 logo: bssSikkimLogo,    href: '#',                               darkBg: false },
  { name: 'PBVM',                fullName: 'Paschim Banga Vigyan Mancha',                logo: pbvmLogo,         href: '#',                               darkBg: true  },
  { name: 'Wild Feathers',       fullName: 'Wildlife · Ecotourism · Adventure',           logo: wildFeathersLogo, href: '#',                               darkBg: false },
];
...
export default function HomePage() {
  ...
  const marqueeItems = [...Array(3)].flatMap(() => PARTNERS);
  ...
  return (
    <div>
      ...
      <section style={{
        ...
        {/* Hero content */}
        <div
          style={{ position: 'relative', maxWidth: 780, margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}
          className="fu"
        >
        ...
        <div style={{
          ...
          {/* Label */}
          <div style={{ textAlign: 'center', fontSize: '.6rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--green)', paddingBottom: '.5rem' }}>
            Our Partners
          </div>
          ...
          <div style={{ overflow: 'hidden', position: 'relative' }}>
            ...
            <div style={{
              ...
              background: 'linear-gradient(90deg, var(--bg3) 0%, transparent 5%, transparent 95%, var(--bg3) 100%)',
            }} />
            ...
            <div style={{
              ...
              {marqueeItems.map(({ name, fullName, logo, href, darkBg }, i) => (
                <a
                  key={i}
                  href={href}
                  target="_blank"
                ...
                >
                  {/* Logo box — 48×48 for clarity, slight padding inside */}
                  <div style={{
                    width: 48,
                    height: 48,
                    borderRadius: 10,
                    overflow: 'hidden',
                    background: darkBg ? '#111' : '#ffffff',
                    flexShrink: 0,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    ...
                    <img
                      ...
                      style={{
                        ...
                        objectFit: 'contain',
                        display: 'block',
                        /* Prevents browser downscaling artefacts */
                        imageRendering: '-webkit-optimize-contrast',
                      }}
                    />
                  </div>

                  {/* Name stacked above full name */}
                  ...
                  {/* Divider */}
                  <span style={{ color: 'var(--border2)', fontSize: '.65rem', marginLeft: '.5rem', opacity: .5 }}>✦</span>
                </a>
              ))}
            </div>
          </div>
      ...
      {state.featuredImagesLoading ? (
        <section style={{ padding: '4rem 1.5rem 5rem', maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ marginBottom: '2.5rem', textAlign: 'center' }}>
            <div className="sec-eye">From the Wild</div>
            <h2 className="sec-h">Featured <em>Captures</em></h2>
          </div>
          <div style={{ height: '75vh', borderRadius: 16, border: '1px solid var(--border)', background: 'var(--bg3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '1rem' }}>
      ...
      <section style={{ padding: '0 1.5rem 5rem', maxWidth: 1200, margin: '0 auto' }}>
        ...
        {state.sightingsError ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', padding: '3rem 2rem', borderRadius: 16, border: '1px solid var(--border)', background: 'var(--bg3)', textAlign: 'center' }}>
