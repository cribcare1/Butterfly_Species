...
//                       }}
//                     >
//                       📍 {parseFloat(img.latitude).toFixed(3)}°N ·{' '}
//                       {parseFloat(img.longitude).toFixed(3)}°E
//                     </div>
//                   )}
//               </div>
//             </div>
...
//       {/* Prev / Next */}
//       {[{ dir: -1, side: 'left', label: '←' }, { dir: 1, side: 'right', label: '→' }].map(
//         ({ dir, side, label }) => (
//           <button
//             key={side}
//             onClick={() => {
...
//               cursor: 'pointer',
//               background:
//                 activeDot === i ? '#fff' : 'rgba(255,255,255,.4)',
//               transition: 'all .3s',
//               padding: 0,
//             }}
//           />
...
//   const { state, dispatch, reload, visibleSightings, sightings } = useApp();
//   const { stats } = state;
...
//           </div>
//           <div style={{
//             height: '75vh', borderRadius: 16, border: '1px solid var(--border)',
  ...
  return (
    <div>
      ...
      <section style={{
        ...
        {/* Hero content — flex:1 so it fills available space and stays centered */}
        <div
        ...
        {/* ── Partners Marquee — inside hero, pinned to bottom ── */}
        <div style={{
          width: '100%',
          marginTop: 'auto',
          ...
          {/* "Our Partners" label */}
          <div style={{ textAlign: 'center', fontSize: '.6rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--green)', paddingBottom: '.4rem' }}>
            Our Partners
          </div>

          {/* Scrolling strip */}
          <div style={{ overflow: 'hidden', padding: '.6rem 0', position: 'relative' }}>
            <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 1, background: 'linear-gradient(90deg, var(--bg3) 0%, transparent 8%, transparent 92%, var(--bg3) 100%)' }} />
            <div style={{ display: 'flex', animation: 'marquee 20s linear infinite', gap: '3rem', width: 'max-content', alignItems: 'center' }}>
              {[...Array(6)].flatMap(() => [
                { name: 'BoRS', fullName: 'Biodiversity of Rabindra Sarovar', logo: borsLogo, href: '#' },
                { name: 'Nature Mates', fullName: 'Nature Club', logo: natureMatesLogo, href: 'https://naturematessociety.org' },
              ]).map(({ name, fullName, logo, href }, i) => (
                <a
                  key={i}
                  href={href}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ display: 'flex', alignItems: 'center', gap: '.6rem', textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}
                >
                  <div style={{ width: 28, height: 28, borderRadius: 6, overflow: 'hidden', background: '#fff', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <img src={logo} alt={name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                  </div>
                  <span style={{ color: 'var(--text2)', fontSize: '.8rem', letterSpacing: '.08em', fontFamily: 'var(--fb)', fontWeight: 600 }}>
                    {name}
                  </span>
                  <span style={{ color: 'var(--text3)', fontSize: '.75rem', letterSpacing: '.06em', fontFamily: 'var(--fb)' }}>
                    — {fullName}
                  </span>
                  <span style={{ color: 'var(--border2)', fontSize: '.7rem', marginLeft: '.5rem' }}>✦</span>
                </a>
              ))}
            </div>
          </div>
